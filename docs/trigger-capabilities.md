...
| Pessoa (`person`) | Disparar notificação quando recebe ligação de contato monitorado | `CallKit` (`CXCallObserver`), `CNContactStore`, `UserNotifications` | Implementado. Monitora ligações recebidas via `CXCallObserver`, mapeia contatos para memórias ativas e dispara notificações imediatas. Suporta contatos com `contactIdentifier` e busca por nome quando necessário. Respeita `SettingsStore.notificationSoundEnabled`. |
| Sequencial (`sequential`) | Agendar automaticamente próxima memória quando anterior é completada | `MemoryService`, `UNUserNotificationCenter` | Implementado. Observa `MemoryService.toggleCompletion` e `setStatus` para detectar conclusão. Quando memória anterior completa, ativa a próxima e cria trigger scheduled para o dia seguinte às 9h. Integrado com `SequentialTriggerExecutor` e `TriggerExecutorCoordinator`. |

## 2. Novos gatilhos com APIs disponíveis
...
| Contatos/Ligações | Após ligação com pessoa marcada, disparar ação | `CallKit`, `CNContactStore` | ✅ **Implementado** no trigger `person`. Considerar extensões de CallKit para melhor identificação de contatos em ligações. |
...
1. ~~**Automação dos triggers atuais**: evoluir pessoa (via CallKit) e sequencial (background + nextFireDate) para alinhar promessa da UI com comportamento real.~~ ✅ **Concluído**: Person e Sequential triggers agora têm automação funcional.
...
5. **Melhorias em Person trigger**: Considerar extensão para detectar mensagens (requer extensão de mensagens) e melhorar identificação de contatos em ligações via CallKit extensions.

## 5. Próximos passos técnicos

- ✅ **Concluído**: Implementação de `PersonTriggerExecutor` com CallKit e `SequentialTriggerExecutor` com automação de agendamento.
- ✅ **Concluído**: Integração com `MemoryService` para notificar conclusão de memórias e disparar sequências.
- ✅ **Concluído**: Configuração de dependências no `TriggerExecutorCoordinator` e `AppEnvironment`.
...
- Melhorar detecção de ligações no `PersonTriggerExecutor`: considerar CallKit extensions para acesso mais preciso ao `contactIdentifier` das ligações.
- Adicionar suporte a detecção de mensagens no `PersonTriggerExecutor` (requer extensão de mensagens do iOS).
