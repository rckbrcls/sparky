...
import type { CreateFolderSheetProps } from "./types";
...
export const CreateFolderSheet: React.FC<CreateFolderSheetProps> = ({
  ...
  const handleCreate = async () => {
    ...
    try {
      setSaving(true);
      await Promise.resolve(onCreate({ name: name.trim(), color, icon }));
      setName("");
    ...
    >
      <BottomSheetView style={[styles.container, { paddingBottom: Math.max(24, 12 + insets.bottom) }]}>
        ...
        >
          <View style={styles.topBar}>
            <View style={styles.heroBadge}>
              <View style={styles.heroIconWrap}>
                <AppIcon icon="folder" size={16} color={Colors.dark.background} />
              </View>
              <Text style={styles.heroBadgeText}>New folder</Text>
            </View>
          ...
          <Text style={styles.subtitle}>Name it and pick a look.</Text>
