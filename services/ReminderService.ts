...
import { addMinutes, addHours, setHours, addDays, parseISO, format } from 'date-fns';
...
export class ReminderService {
  ...
  static async createReminder(reminderData: {
    ...
    const nextFireAt = this.calculateNextFireAt(reminderData.fireAt, reminderData.rrule);
  ...
  static async updateReminder(
    ...
    if (updates.fireAt !== undefined) {
      nextFireAt = this.calculateNextFireAt(updates.fireAt, updates.rrule || reminder.rrule);
    }
  ...
  static async snoozeReminder(reminderId: string): Promise<void> {
    ...
    const snoozeHistory = await database.getSnoozeHistoryForReminder(reminderId);
  ...
  static async remindLater(reminderId: string): Promise<void> {
    ...
    const currentStage = Math.min(reviewStage.currentStage + 1, this.REVIEW_INTERVALS.length - 1);
  ...
  private static calculateNextFireAt(fireAt?: Date, rrule?: string): Date | undefined {
  ...
  private static async generateLeadTimeReminders(
    ...
    for (const leadTimeDays of dateData.leadTimes) {
      ...
      if (reminderDate > now) {
        const title = leadTimeDays < 1 
          ? `${dateData.title} em ${Math.round(leadTimeDays * 24)} horas`
          : `${dateData.title} em ${leadTimeDays} ${leadTimeDays === 1 ? 'dia' : 'dias'}`;
  ...
  static async getRemindersByPersonOrProject(filter: string): Promise<Reminder[]> {
  ...
  static async exportCSV(): Promise<string> {
    ...
    const rows = reminders.map(reminder => [
      ...
      reminder.location || '',
      reminder.type,
      reminder.nextFireAt ? format(parseISO(reminder.nextFireAt), 'dd/MM/yyyy HH:mm') : '',
      reminder.status,
